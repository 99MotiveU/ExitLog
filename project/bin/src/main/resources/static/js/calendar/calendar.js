	...
	try {
		...
		allJobPosts.forEach(job => {
			...
			for (let m = moment(openingDateMoment); m.isSameOrBefore(expirationDateMoment, 'day'); m.add(1, 'days')) {
				...
				let displayCompanyText = job.companyName || '';
				...
				processedCalendarEvents.push({
					...
					extendedProps: {
						company: job.companyName, // 원본 회사명 (모달용)
		...
		console.log('[Processed Calendar Events for FC]', processedCalendarEvents);
...
document.addEventListener('DOMContentLoaded', async function() {
	...
	calendar = new FullCalendar.Calendar(calendarEl, {
		...
		navLinks: true,
		dayMaxEventRows: true, // "+N more" link 활성화는 유지
		                    ...
		                    <div class="modal-dialog">
		                        <div class="modal-content">
		                            ...
		                            <div class="modal-body">
		                                ...
		                                <a href="${event.extendedProps.postUrl}" target="_blank" class="btn btn-primary">공고 보기</a>
		                            </div>
		                        </div>
		                    </div>
		                </div>
